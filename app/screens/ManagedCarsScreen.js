...
const ManagedCarsScreen = ({ navigation, route }) => {
  ...
  // Car data state
  const [managedCars, setManagedCars] = useState([]);
  ...
  useEffect(() => {
    const loadManagedVehicles = async () => {
      ...
      try {
        console.log('Loading managed vehicles...');
        const response = await managedVehiclesService.getManagedVehicles(50); // Get more managed vehicles
        ...
        if (vehiclesData && vehiclesData.length > 0) {
          ...
          setManagedCars(transformedVehicles);
          setFilteredCars(transformedVehicles);
        } else {
          console.log('No managed vehicles data');
          setManagedCars([]);
          setFilteredCars([]);
        }
      } catch (error) {
  ...
  const handleSearch = (text) => {
    ...
    if (text.trim() === '') {
      setFilteredCars(managedCars);
    } else {
      const searchLower = text.toLowerCase();
      const searchResults = managedCars.filter(car => {
        ...
        <View style={{ position: 'absolute', top: insets.top , left: 0, right: 0, zIndex: 200 }}>
          <View style={styles.filterBar}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[styles.topTabs, { direction: 'ltr' }]}>
              ...
              <TouchableOpacity style={styles.filterBtn}><Text style={styles.filterBtnText}>Managed</Text></TouchableOpacity>
            </ScrollView>
          </View>
          <View style={styles.resultsRow}>
            <Text style={styles.resultsText}>{managedCars.length} managed results</Text>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerSearchWrapper}>
            <TextInput
              ...
              placeholder="Search managed cars"
        ...
        <View style={styles.resultsRow}>
          <Text style={styles.resultsText}>
            {isSearching ? `Search results: ${filteredCars.length}` : `${filteredCars.length} managed results`}
          </Text>
          <TouchableOpacity><Text style={styles.saveSearch}>Save Search</Text></TouchableOpacity>
        </View>
        ...
        {apiError && !isLoading && (
          <View style={styles.errorContainer}>
            ...
            <Text style={styles.errorSubtext}>Unable to load managed vehicles</Text>
          </View>
        )}
        ...
        {!isLoading && !apiError && filteredCars.length === 0 && (
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>No managed vehicles found</Text>
          ...
          >
            ...
            <View style={styles.badgesRow}>
              <View style={styles.managedBadgeRow}>
                <Image source={require('../assets/images/badge.png')} style={styles.managedBadgeIcon} />
                <View style={styles.managedBadgeTextBg}>
                  <Text style={styles.managedBadgeText}>MANAGED BY PAKWHEELS</Text>
                </View>
              </View>
            </View>
          </TouchableOpacity>
      ...
      <AuthModal
        ...
        onSignIn={() => {
          ...
          navigation.navigate('SignInScreen', {
            returnScreen: 'ManagedCarsScreen',
...
const styles = StyleSheet.create({
  ...
  managedBadgeIcon: { width: 22, height: 22, marginRight: 4, resizeMode: 'contain' },
  managedBadgeTextBg: { backgroundColor: '#f3f4f6', paddingHorizontal: 6, borderRadius: 6, alignSelf: 'center', marginLeft: -4 },
  managedBadgeText: { color: '#222222', fontWeight: '700', fontSize: 11 },
...
export default ManagedCarsScreen; 